...
| P2.3 | `verification-script` | Entry point + orchestration + script generation | Sonnet | **Yes** |
...
P2.3 (`verification-script`) is the **user-facing entry point** that provides orchestration instructions. When invoked via `/verification-script` or by pipeline:
...
```
User: /verification-script src/auth/         OR   test-audit Stage 7
                    ↓
┌─────────────────────────────────────────────────────────────────────┐
...
The skill MUST:
- Be user-invocable via `/verification-script [path]`
...
Logs written to:
- `logs/verification-script-{YYYYMMDD-HHMMSS}.yaml`
- `logs/diagnostics/verification-script-{YYYYMMDD-HHMMSS}.yaml`
...
### P2.3: verification-script Skill

**Location:** `skills/verification-script/SKILL.md`
...
**Load this skill when:**
- User requests `/verification-script [path]`
...
```
/verification-script [path] [--execute]
```

**Examples:**
- `/verification-script src/cli.ts` - Generate verification script for CLI
- `/verification-script src/server.ts --execute` - Generate and run
- `/verification-script` - Infer from recent context
...
Write log to: `logs/verification-script-{YYYYMMDD-HHMMSS}.yaml`

Write diagnostics to: `logs/diagnostics/verification-script-{YYYYMMDD-HHMMSS}.yaml`
...
if [[ "$FILE_PATH" =~ \.(test|spec)\. ]]; then
  # Test file change
  PIPELINE="Verification: Run /verification-script on changed tests, then execute"
else
  # Production code change
  PIPELINE="TDD: Write tests for changes, run /verification-script, execute"
fi
```
...
- [ ] `/verification-script src/component.ts` generates executable script
...
### SC5: Menu Visibility
- [ ] `verification-script` appears in `/` menu
...
/anthropic-validator skills/verification-script
```
...
Create in `tests/fixtures/verification-script/`:

### Fixture 1: Node CLI Component
```
tests/fixtures/verification-script/node-cli/
├── package.json
├── src/
│   └── cli.ts
└── expected/
...
```
tests/fixtures/verification-script/node-http/
├── package.json
├── src/
│   └── server.ts
└── expected/
...
```
tests/fixtures/verification-script/python-parser/
├── pyproject.toml
├── src/
│   └── parser.py
└── expected/
...
```
tests/fixtures/verification-script/generic-spawner/
├── spawner.sh
└── expected/
    └── spawner-verify.sh
```
...
**Purpose**: Verify verification-script (P2.3) correctly generates executable scripts using assertion-patterns (P2.1) and component-patterns (P2.2).
...
1. Ensure skills are copied to `.claude/skills/`:
   ...
   - `verification-script/SKILL.md`
2. Ensure test fixtures exist in `tests/fixtures/verification-script/`
3. Ensure `tmp/verification/` directory exists
4. Start a fresh Claude Code session
5. Verify `/skills` shows `verification-script` (user-invocable: true)
...
**Prompt**:
```
/verification-script tests/fixtures/verification-script/node-cli/src/cli.ts
```
...
- [ ] Log written to `logs/verification-script-*.yaml`
...
**Prompt**:
```
/verification-script tests/fixtures/verification-script/python-parser/src/parser.py
```
...
**Expected Behavior**:
- `verification-script` SHOULD appear in the menu
- `assertion-patterns` should NOT appear
- `component-patterns` should NOT appear

**Verification**:
- [ ] `verification-script` in menu (user-invocable: true)
...
```yaml
# tests/logs/verification-script-test-results-YYYYMMDD.yaml
...
1. Create test fixtures in `tests/fixtures/verification-script/`
2. Implement P2.1 `assertion-patterns`
3. Implement P2.2 `component-patterns`
4. Implement P2.3 `verification-script`
...
| Task | Status | Notes |
|------|--------|-------|
| P2.1 assertion-patterns | Not started | |
| P2.2 component-patterns | Not started | |
| P2.3 verification-script | Not started | |
| test-audit Step 7 update | Not started | |
| Manual testing | Not started | Run in separate session |
| Anthropic validation | Not started | |
