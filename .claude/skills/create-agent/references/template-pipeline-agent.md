...
5. **REQUIRED**: Return pipeline suggestions with MANDATORY language (SA6)
...
- Write all sub-agent output to `logs/` (SA2)
- Read previous stage output from `logs/` before spawning next stage (SA4)
...
- Spawn sub-agents with `run_in_background: true` (SA5)
...
Include: stages completed, key findings per stage, report paths, pipeline suggestions (SA6).
...
- [ ] Pipeline suggestions included in summary (SA6)
...
- Each stage writes to `logs/` — next stage reads from there (SA2/SA4)
