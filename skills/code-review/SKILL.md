...
description: >
  ...
  pipeline agents. Two-phase workflow: static analysis first, then LLM judgment.
user-invocable: true
agent: sonnet
skills:
  - subagent-prompting
...
**CRITICAL**: Always run Phase 1 before Phase 2. Do not skip.
...
- Unsafe assertion patterns (`as unknown as T`, `!` operator)
...
After completing review, write diagnostic output to:
```
