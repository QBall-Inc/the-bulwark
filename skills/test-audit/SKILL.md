...
**Mitigation:** Integration/e2e files are always flagged for deep analysis regardless of mock indicators. Future enhancement: AST-based data flow analysis (deferred to P5.x).
...
**Mitigation:** Two-gate logic uses P0/P1 priority as primary trigger, with 95% threshold as secondary. Future enhancement: AST-based verification counter (deferred to P5.x).

### T4 Detection Not Implemented

**Issue:** T4 (skipped tests, `.only`, `.skip` markers) detection is not automated.

**Mitigation:** T4 noted in rules but flagged for manual review. Future enhancement: Skip detector script (deferred to P5.x).
...
**Mitigation:** Integration/e2e files are always flagged regardless of indicators. Future enhancement: Extended pattern detection for common stub conventions (deferred to P5.x).
